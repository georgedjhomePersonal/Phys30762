...
#include <limits>
double BohrEnergy(){
    // prompt user for input and validate it, throwing exceptions for invalid input
    int z, n_i, n_f;
    std::string unit;
...
int main(){
    // loop to allow user to continue/retry after an exception is caught
    std::string cont{"y"};
    while (cont == "y"){
        // try catch block to handle exceptions thrown by BohrEnergy function
        try{
